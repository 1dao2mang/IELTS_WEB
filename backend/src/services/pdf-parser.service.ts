...
export class PDFParserService {
  ...
  async debugPDFStructure(pdfPath: string) {
  ...
  async importTestsToDatabase(pdfPath: string) {
  ...
  private async parseTestsFromText(text: string): Promise<ParsedTest[]> {
  ...
  private parsePassages(text: string): ParsedPassage[] {
  ...
  private parseQuestions(text: string): ParsedQuestion[] {
    ...
    const mainRanges: Array<{ start: number; end: number }> = [];
  ...
  private async importTestToDb(parsedTest: ParsedTest) {
