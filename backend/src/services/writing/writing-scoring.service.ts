...
export interface WritingScores {
  overall: number;
  taskAchievement: number; // Task 1: TA | Task 2: TR (Task Response)
  coherence: number;
  lexical: number;
  grammar: number;
}
  ...
  async evaluateWriting(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        // Parse JSON response (same robust parsing as Speaking)
        try {
          response = response
            .trim()
            .replace(/^```json\s*/i, "")
...
- Be realistic with scores (avoid 8.5+)`;
   ...
   */
  async calculateBandScore(content: string, taskType: string) {
